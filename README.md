...
3. Em Authentication → URL Configuration, configure Site URL `http://localhost:3000` e permita `http://localhost:3000/auth/confirm`. Na publicação use o domínio HTTPS e ajuste `APP_URL`.
...
Gemini, Hotmart e WhatsApp estão desativados até configuração e teste. WhatsApp só tem recepção de eventos; banco real e OCR/PDF não estão implementados. O manifesto permite instalação, sem operação financeira offline. Textos legais são rascunhos.
