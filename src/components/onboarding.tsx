...
export function Onboarding(){
 const [step,setStep]=useState(1),[country,setCountry]=useState('MX'),[name,setName]=useState(''),[localError,setLocalError]=useState('');
 const [state,action,pending]=useActionState(completeOnboarding,{});
 function goToFinancialStep(){
  if(!name.trim()){setLocalError('Escribe tu nombre para continuar.');return;}
 ...
 <fieldset hidden={step!==1}><h1>Bienvenido a MAYORDOMO</h1><p className="lead">Tu dinero tiene una historia.<br/>Vamos a entenderla juntos.</p><Label htmlFor="name">¿Cómo te llamas?</Label><Input id="name" name="name" maxLength={80} autoComplete="name" value={name} onChange={event=>setName(event.target.value)}/>{localError&&<p role="alert" className="error-message">{localError}</p>}<Button type="button" onClick={goToFinancialStep}>Comenzar</Button></fieldset>
