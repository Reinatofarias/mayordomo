...
export default async function Profile(){
 ...
 return <>
  ...
  <section className="profile-hero"><div><p className="eyebrow">{t.context}</p><h2>{t.heroTitle}</h2><p>{t.heroText}</p></div>{principle&&<PrincipleCard reference={principle.reference} principle={principle.principle} application={principle.application} riskContext={principle.risk_context}/>}</section>
  ...
  <section className="section"><h2>{t.billing}</h2>{!access&&<p className="status-warning">{t.accessWarn}</p>}{subscriptions?.length?subscriptions.map(s=><article key={s.id} className="billing-card"><h3>{s.plan}</h3><p>{t.provider}: {s.provider} - {t.status}: {s.status}</p><p>{t.purchase}: {fallbackDate(s.purchased_at,c.profile.locale)}<br/>{t.renewal}: {fallbackDate(s.renewal_at,c.profile.locale)}<br/>{t.accessUntil}: {fallbackDate(s.access_until,c.profile.locale)}</p><Button asChild variant="outline"><a href="https://consumer.hotmart.com/" target="_blank" rel="noopener noreferrer">{t.manage}</a></Button></article>):<p>{t.noSubscription}</p>}<div className="page-actions"><form action={refreshHotmartAccess}><Button variant="outline">{t.checkHotmart}</Button></form>{checkout&&<Button asChild><a href={checkout} target="_blank" rel="noopener noreferrer">{t.buy}</a></Button>}</div><p className="hint">{t.billingHint}</p><Link href="#soporte">{t.askHelp}</Link></section>
  <section className="section" id="soporte"><h2>{t.supportTitle}</h2><p>{t.supportText}</p>{requests?.length?<div className="support-history"><h3>{t.recent}</h3>{requests.map(r=><article key={r.id}><strong>{r.subject}</strong><span>{r.status==='OPEN'?t.received:statusText(r.status,c.profile.locale)} - {fallbackDate(r.created_at,c.profile.locale)}</span></article>)}</div>:<p className="hint">{t.noSupport}</p>}</section>
