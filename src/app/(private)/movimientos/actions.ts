...
export async function saveTransaction(_previous:FormState,form:FormData):Promise<FormState>{
 ...
 const result=editing?await c.db.from('transactions').update(record).eq('id',d.id).eq('user_id',c.user.id).select('id').single():await c.db.from('transactions').insert({...record,id:d.id,user_id:c.user.id,source:'MANUAL',external_id:d.id}).select('id').single();
 if(result.error){if(result.error.code!=='23505')return {error:'No pudimos guardar el movimiento. Inténtalo de nuevo.'};}
 revalidatePath('/hoy');revalidatePath('/movimientos');redirect('/movimientos?guardado='+(editing?'actualizado':'creado'));
}
export async function deleteTransaction(_previous:FormState,form:FormData):Promise<FormState>{
 ...
 if(error)return {error:'No pudimos eliminar el movimiento.'};revalidatePath('/hoy');revalidatePath('/movimientos');redirect('/movimientos?guardado=eliminado');
}
