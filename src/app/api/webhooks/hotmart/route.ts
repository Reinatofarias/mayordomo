...
const policySchema=z.object({productIds:z.array(z.string().min(1)).min(1),accessMode:z.enum(['FIXED_DAYS','PROVIDER_PERIOD']),accessDays:z.number().int().min(1).max(3660).optional()});
function payloadSummary(payload:unknown){
 if(!payload||typeof payload!=='object')return {};
 const event=(payload as {event?:unknown}).event;
 const data=(payload as {data?:unknown}).data;
 const product=data&&typeof data==='object'?(data as {product?:unknown}).product:null;
