...
export async function authenticate(_previous:FormState,form:FormData):Promise<FormState> {
 ...
 if(mode==='signup') {
  ...
  const {error}=await db.auth.signUp({...parsed.data,options:{emailRedirectTo:new URL('/auth/confirm',process.env.APP_URL!).toString(),data:{terms_version:'draft-1'}}});
...
export async function recover(_previous:FormState,form:FormData):Promise<FormState> {
 ...
 const db=await createClient();
 await db.auth.resetPasswordForEmail(email.data,{redirectTo:new URL('/auth/confirm?next=recovery',process.env.APP_URL!).toString()});
